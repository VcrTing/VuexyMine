...
export const paginationMetaDan = computed(() => {
  return <T extends { page: number; pageSize: number }>(options: T, total: number) => {
    const start = (options.page - 1) * options.pageSize + 1
    const end = Math.min(options.page * options.pageSize, total)
